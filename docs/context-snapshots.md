...
- **Built by** `.aidd/framework/scripts/build-snapshot.sh <tag>`, which writes
  ...
  `quality-baseline.md` (measured sigmas — test-file count, repo size, complexity
  hotspots, TODO/FIXME count, coverage %, lint status (measured when architecture.md
  declares canonical commands; explicit `na` otherwise) — each with its command), and
  `delta.md` (churn since the previous snapshot), plus a gitignored `.last-ref` marker.
...
- **History trail** — every rebuild copies its three files to
  ...
  reviewer (`mode=delta`) compares the pre-implementation pack — the latest
  `pre-construction` history pack, falling back to `pre-inception` — against the
  current one.
