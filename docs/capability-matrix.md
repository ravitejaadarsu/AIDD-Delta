...
| **Plugin command surface `/aidd:*`** (`commands/`) | **supported** — 21 slash commands (`/aidd:go`, `/aidd:approve`, `/aidd:status`, `/aidd:review-pr`, …) | **degraded** — natural-language routing through the `AGENTS.md` managed block: `AIDD: build <intent>`, `AIDD: status`, `AIDD: approve` | **degraded** — paste the matching prompt from `.aidd/framework/prompts/`; approvals can also be recorded by editing `state.yaml` per `core/protocol/gates.md` |
...
| **External PR review** (`core/protocol/pr-review.md`) | **supported** — the phase-1 finders (one per changed source file, sweeps, dimension specialists) and the phase-2 per-finding adversarial verifiers run concurrently as Task-tool subagents | **degraded** — the identical units run sequentially in the documented order (path asc, finding id asc); every phase, every verdict, and both the report and comment artifacts are unchanged | **degraded** — same sequential order, each dispatch pasted by the operator; platform fetch degrades to the operator supplying the PR refs, and the merge-base diff is computed locally exactly as at Tier 1 |
